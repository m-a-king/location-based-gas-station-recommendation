...
| 2 | 기준가 여과 | 가격이 기본 경로상 최저가를 초과하지 않는 주유소만 보존 |
| 3 | 호출 수 축소 | 가격 오름차순 상위 30개 주유소만 유지 |
...
탐색을 종료해도 반환 결과의 최적성이 보존된다. 특히 경로상 후보는 우회 비용이 0에 가까워 점수가 가격에 비례하므로 자연스러운 기준선을 형성하며, 본 단계적 압축은 이 기준선을 2단계 기준가 여과에서 직접 활용한다.
