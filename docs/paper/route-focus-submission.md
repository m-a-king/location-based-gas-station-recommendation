...
기존 서비스는 기본 경로상의 주유소만 후보로 삼는다. 그러나 기본 경로에서 다소 떨어져 보이는 주유소라도, 그 주유소를 경유지로 두고 출발지에서 도착지까지의 경로를 새로 계산하면 도로망에 따라 새 경유 경로가 기본 경로보다 크게 길어지지 않는 경우가 있다. 가격 차이가 충분히 크다면 이런 후보가 더 경제적인 선택이 된다. 본 연구는 기본 경로를 감싸는 MBR과 여유 영역까지 탐색 공간을 확장하고, 각 후보를 경유지로 둔 경로를 새로 계산해 기본 경로와 경유 경로 간 거리·시간 차이를 우회 비용으로 측정하여 순위를 결정한다.
...
기본 경로로부터의 직선거리만으로 후보를 거르는 방식은 한계가 분명하다. 직선상 멀어 보여도 경유 경로를 새로 계산하면 우회 비용이 작은 후보가 발견되는데, 이런 후보들이 사전에 배제되면 검색에서 누락되는 오류가 발생한다. 본 연구는 탐색 공간을 관대하게 열되, 후보 수의 추이를 관찰하며 가격 하한과 호출 수 상한을 결합한 단계적 점진 압축으로 좋은 후보를 보존하면서 외부 경로 탐색 API 호출 수를 제어한다. 단계 구성은 표 2와 같다.
...
경유 재조회는 각 후보의 실측 우회 비용을 외부 경로 탐색 API로 산출해 식 (1)의 점수를 매기는 과정이며, API 호출을 동반하므로 가능한 최소로 수행해야 한다. 본 시스템은 후보를 가격 오름차순으로 살피며, 추천 개수 $k$만큼 상위 후보를 확보한 뒤부터 종료 조건을 검사한다. 새 후보의 가격 하한 $p_i \ell$이 확보된 상위 $k$개 중 가장 나쁜 점수 $\text{score}^{(k)}_{\text{best}}$를 초과하면, 즉 식 (3)이 성립하면 탐색을 즉시 종료한다.
...
본 논문은 기본 경로상으로 후보를 한정하는 기존 방식의 한계를 지적하고, 경유 경로 재탐색을 통한 비용 통합 점수로 주유소 추천을 제안하였다. 또한 입지 최적화의 deviation tolerance[4]를 운용 시점 추천으로 전환하였으며, MBR 단계적 압축과 가격 하한 가지치기로 외부 경로 탐색 API 호출 수를 제어하면서도 최적성을 보존하는 알고리즘을 정식화하였다. 실측 사례 4개 시나리오 모두에서 기존 서비스 대비 가격 우위(최대 77원/L)를 확인하였으며, 출발지·도착지 입력부터 외부 내비게이션 앱 연동까지를 포함한 운용 가능한 형태로 구현하여 사용자가 추천 결과를 즉시 활용할 수 있도록 하였다. 본 시스템은 경유 경로 재조회를 외부 경로 탐색 API에 의존하므로 호출 비용과 응답 지연이 운용상의 제약으로 남으며, 위 알고리즘이 이를 부분적으로 완화한다. 향후 시간대·요일에 따른 우회 비용 변동을 누적 데이터로 반영하고, 정적 우회 허용 상한과 [5]의 평균 수용 우회 범위를 사용자의 채택·거부 신호로 학습하여 개인화 추천으로 확장할 계획이다.
