...
3. 경유 재조회: 각 후보를 경유지로 하는 경로를 Kakao Directions API로 재조회해 실측 우회 거리·시간을 얻으며, 가격 하한 가지치기로 호출 수를 제한한다(3.5절).
...
기본 경로로부터의 직선 거리만으로 후보를 거르는 방식은 한계가 분명하다. 직선상 멀어 보여도 경유 경로를 새로 계산하면 우회 비용이 작은 후보가 적지 않게 발견되는데, 이런 후보들이 사전에 배제되면 검색에서 누락되는 오류(false negative)가 발생한다. 본 연구는 탐색 공간을 관대하게 열되, 후보 수의 추이를 관찰하며 가격 하한과 호출 수 상한을 결합한 단계적 점진 압축으로 좋은 후보를 보존하면서 외부 API 호출을 제어한다(표 2).
...
| 단계 | 명칭 | 동작 |
|:---:|---|---|
| 1 | 경로 MBR 후보 수집 | 기본 경로의 MBR + 동적 여유 영역 내 주유소 수집 |
| 2 | 기본 경로상 최저가 기반 가격 상한 | 가격이 기본 경로상 최저가를 초과하지 않는 주유소만 보존 |
| 3 | 외부 API 호출 수 상한 | 가격 오름차순 상위 30개 주유소만 유지 |
...
점수가 $p_i \ell$ 이상이므로 가격이 경로상 최저가를 넘는 후보는 우회 비용을 0으로 가정해도 경로상 최저가 후보를 이길 수 없다. 다만 경로상 후보가 한 곳도 없는 시나리오에서는 이 상한을 적용하지 않는다. 끝으로 3단계는 안전장치다. 잔여 후보가 여전히 많을 경우 가격 오름차순 상위 30개로 잘라, 외부 API 호출을 30회 이내로 제한한다. 이렇게 식 (2)의 수학적 하한과 deviation tolerance[4] 개념이 한 흐름 안에서 결합되어, 호출 비용을 관리하면서도 우수한 후보를 보존한다.

### 3.5 가격 하한 가지치기

단계 3의 경유 경로 호출은 외부 API에 의존하므로 가능한 최소로 수행해야 한다. 식 (2)의 하한을 활용하면 다음과 같은 종료 조건이 성립한다. 후보를 가격 오름차순으로 탐색하면서 상위 $k$개가 확보된 이후 어느 시점에서든 다음의 (3)이 성립하면,

$$
p_i \ell \;>\; \text{score}^{(k)}_{\text{best}} \tag{3}
$$

탐색을 종료해도 반환 결과의 최적성이 보존된다. 특히 경로상 후보는 우회 비용이 0에 가까워 점수가 가격에 비례하므로 자연스러운 기준선을 형성하며, 본 단계적 압축은 이 기준선을 2단계 기본 경로상 최저가 기반 가격 상한에서 직접 활용한다.
