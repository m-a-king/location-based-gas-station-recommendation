...
2. **후보 수집**: 기본 경로의 MBR에 우회 허용 상한의 두 배 buffer를 더해 탐색 공간을 구성하고 내부 주유소를 DB에서 조회한다. 후보가 과다하면 단계적으로 압축한다(3.4절 참조).
...
| 단계 | 명칭 | 동작 |
|:---:|---|---|
| 1 | 경로 MBR 후보 수집 | 기본 경로의 MBR + 동적 buffer(우회 상한 × 2) 내 주유소 수집 |
