...
경로상 후보 식별을 거리 기준으로만 자르면 직선상 멀지만 실제 우회 비용이 작은 후보를 놓치는 누락 오류(false negative)를 유발하고, 반대로 공간을 너무 넓게 잡으면 단계 3 API 호출이 폭증한다. 본 연구는 후보 수를 관찰하며 식 (2)의 가격 하한과 외부 API 호출 수 강제 상한을 결합해 단계적으로 점진 압축하는 전략을 채택한다(표 2).
