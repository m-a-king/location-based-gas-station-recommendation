...
(3)이 성립하면 탐색을 종료해도 반환 결과의 최적성이 보존된다. 특히 경로상 후보는 우회 비용이 0에 가까워 점수가 가격에 비례하므로 자연스러운 기준선을 형성하며, 본 단계적 압축은 이 기준선을 2단계 경로상 최저가 기반 가격 상한에서 직접 활용한다.
...
본 논문은 기본 경로상으로 후보를 한정하는 기존 방식의 한계를 지적하고, 경유 경로 재탐색을 통한 비용 통합 점수로 주유소 추천을 재정식화하였다. 입지 최적화의 deviation tolerance[5]를 운용 시점 추천으로 전환하였으며, 실측 사례에서 도심 시나리오의 사각지대 보완과 기존 서비스 대비 가격 우위를 확인하였다. 본 시스템은 출발지·도착지 입력 UI와 선택된 경유 경로의 외부 내비게이션 앱 연동을 포함하여, 추천 결과를 사용자가 즉시 운행에 활용할 수 있는 형태로 구현되었다. 향후 여유 영역의 폭과 경로상 후보 식별 기준 결정 규칙의 데이터 기반 학습, 주유소 밀도 지표를 활용한 단계적 압축의 분기 최적화, 개별 운전자의 수용 우회 범위 개인화 학습으로 확장할 수 있다.
