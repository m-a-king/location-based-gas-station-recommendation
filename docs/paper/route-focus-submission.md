...
본 논문은 경로 기반 주유소 추천에서 탐색 공간을 경로 주변에 한정하지 않고 그 경로를 포함하는 최소경계사각형(MBR) 내부로 확장하는 2-hop 재탐색 방식을 제안한다. 후보가 과도하면 기본 경로 근접도와 가격으로 단계 압축한 뒤, 각 후보를 경유하는 실제 도로 경로를 재산출해 주유비와 우회의 연료·시간 비용을 합산한 점수로 순위를 정한다. 가격 오름차순 탐색 중 남은 후보로 지출을 낮출 수 없는 시점에 탐색을 중단하여 외부 API 호출을 절제하면서 상위 추천의 최적성을 보존하고, 경로상 단순 최저가 선택 대비 실질 지출 절감을 달성한다.
...
국소 탐색은 폴리라인으로부터의 직선 거리를 기준으로 후보를 필터링하지만, 도로망에서는 직선 거리와 실제 진출입 우회 거리가 크게 어긋날 수 있다. 예를 들어 폴리라인에서 직선 3 km 떨어진 주유소가 진출입 구조상 500 m 우회로 접근되는 사례가 자주 관찰된다(그림 2). 본 연구는 탐색 범위를 경로를 포함하는 최소경계사각형(MBR)으로 확장하고, 내부 후보 각각에 대해 경로를 재조회해 실측 우회 비용으로 순위를 정한다.

[그림 2] 폴리라인 MBR 확장과 직선 거리–실제 우회 거리의 차이

### 3.2 2-hop 재탐색 프레임워크

제안 시스템은 네 단계로 동작한다. "2-hop"은 기본 경로 조회 1회와 경유 경로 재조회 1회, 즉 두 번의 경로 조회로 각 후보의 점수를 확정한다는 의미이다.
...
3. **경유 재조회**: 각 후보를 경유지로 하는 경로(origin → candidate → destination)를 Kakao API로 재조회해 실측 우회 거리·시간을 얻으며, 가격 하한 pruning으로 호출 수를 제한한다(3.5).
...
고정 corridor(예: 경로로부터 직선 2 km) 방식은 false negative를 유발하고, 공간을 너무 넓게 잡으면 단계 3 API 호출이 폭증한다. 본 연구는 후보 수 $N$을 관찰하며 cascade로 점진 압축하는 전략을 채택한다(표 2).
...
1단계 POLYLINE_MBR이 공간을 관대하게 열고, 2단계 TIGHT_CORRIDOR는 기본 경로 근접도로 1차 압축해 MBR 내부 dead-zone을 제거한다. 여기서 쓰는 2 km 임계값은 [5]가 보고한 운전자 수용 우회 범위의 직선거리 근사에 해당한다. 3단계 PRICE_CAPPED는 가격 결합 후에도 과다한 후보를 가격 상위로 2차 압축해 단계 3 호출 수를 30회 이하로 제한한다. 이 cascade는 deviation tolerance[4]를 호출 예산 관리로 번역한 결과로 볼 수 있다.
...
표 3은 **⟨실측 교체 예정: 수집 일자 YYYY-MM-DD, 대상 지역⟩**에서 오피넷과 오일나우를 통해 수집한 실측 가격과 Kakao Mobility Directions 응답을 바탕으로, 세 건의 기본 경로에 대해 경로상 최저가 후보(ON_ROUTE)와 비용 인지 우회 후보(OFF_ROUTE)를 비교한 결과이다(유종: 휘발유, $\ell = 40$ L, $\eta = 10$ km/L). 각 사례의 우회 허용 상한은 $\min(\text{기본경로} \times 0.3,\ 10)$ km로 산정하였다.

표 3. 실측 기반 사례 비교
Table 3. Empirical case comparison

| 사례 | 기본경로 | 후보 | 가격(원/L) | 우회(km) | 점수 | 절감(원) |
|---|---:|---|---:|---:|---:|---:|
| **⟨사례 1: 출발–도착⟩** | **⟨? km⟩** | ON_ROUTE | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** | — |
| **⟨사례 1: 출발–도착⟩** | **⟨? km⟩** | OFF_ROUTE | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** |
| **⟨사례 2: 출발–도착⟩** | **⟨? km⟩** | ON_ROUTE | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** | — |
| **⟨사례 2: 출발–도착⟩** | **⟨? km⟩** | OFF_ROUTE | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** |
| **⟨사례 3: 출발–도착⟩** | **⟨? km⟩** | ON_ROUTE | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** | — |
| **⟨사례 3: 출발–도착⟩** | **⟨? km⟩** | OFF_ROUTE | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** | **⟨?⟩** |

세 사례 모두 우회 후보가 경로상 최저가 대비 지출을 낮췄으며, **⟨실측 교체 예정: 평균 절감 ₩?, 최대 절감 ₩?⟩**의 효과를 보였다. 각 사례에서 우회 상한을 초과하는 후보는 가격과 무관하게 배제되어 비현실적 우회가 추천되지 않았다.

## Ⅳ. 결론

본 논문은 경로상 국소 탐색의 한계를 지적하고, 주유소 추천을 비용 인지 우회를 통한 2-hop 재탐색으로 재정식화하여 지출 최소화 관점의 추천을 실현하였다. 입지 최적화의 deviation tolerance[4]를 운용 시점 추천으로 전환하였으며, 실측 사례에서 경로상 최근접 방식 대비 평균 **⟨실측 교체 예정: ₩?⟩**의 실질 지출 절감을 확인하였다. 향후 MBR buffer·corridor 거리의 이동 거리 기반 동적 스케일링, 주유소 밀도 지표를 활용한 cascade 분기 최적화, 개별 운전자의 수용 우회 범위 개인화 학습으로 확장할 수 있다.
