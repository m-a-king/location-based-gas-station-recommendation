...
표 3에서 본 시스템은 4개 시나리오 모두에서 기존 서비스 대비 가격 우위를 보였다. 우위 폭은 10~77원/L 범위였으며, 가장 큰 사례 4의 경우 50 L 기준 약 3,850원의 절감에 해당한다. 기존 서비스에서 옵션 조합으로 근접한 결과를 얻을 수 있는 경우라도 본 시스템은 별도 조작 없이 비용 통합 점수로 후보를 직접 제시한다는 점에서 자동화 측면의 차별성을 가진다.
...
본 논문은 기본 경로상으로 후보를 한정하는 기존 방식의 한계를 지적하고, 경유 경로 재탐색을 통한 비용 통합 점수로 주유소 추천을 제안하였다. 본 연구는 입지 최적화의 deviation tolerance[4]를 운용 시점 추천으로 전환하였으며, MBR 단계적 압축과 가격 하한 가지치기로 외부 경로 탐색 API 호출 수를 제어하면서도 최적성을 보존하는 알고리즘을 정식화하였다. 실측 사례 4개 시나리오 모두에서 기존 서비스 대비 가격 우위(최대 77원/L)를 확인하였으며, 출발지·도착지 입력부터 외부 내비게이션 앱 연동까지를 포함한 운용 가능한 형태로 구현하여 사용자가 추천 결과를 즉시 활용할 수 있도록 하였다. 본 연구는 경유 경로 재조회를 외부 경로 탐색 API에 의존하므로 호출 비용과 응답 지연이 운용상의 제약으로 남으며, 본 연구의 알고리즘이 이를 부분적으로 완화한다. 향후 시간대·요일에 따른 우회 비용 변동을 누적 데이터로 반영하고, 정적 우회 허용 상한과 [5]의 평균 수용 우회 범위를 사용자의 채택·거부 신호로 학습하여 개인화 추천으로 확장할 계획이다.
