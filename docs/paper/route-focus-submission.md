...
본 논문은 주유소를 들렀다 가는 경로 추천에서 탐색 공간을 경로 주변으로 한정하던 기존 방식 대신, 경로를 포함하는 최소경계사각형(MBR)까지 탐색 공간을 확장해 최적의 주유 지점을 효율적으로 찾는 방법을 제안한다. 후보가 너무 많을 때는 경로상 후보의 최저가를 기준으로 더 비싼 후보를 미리 잘라낸 뒤, 각 후보를 경유하는 실제 도로 경로를 다시 계산해 주유비와 우회로 인한 연료·시간 비용을 합산한 점수로 순위를 매긴다. 가격이 낮은 순서로 살펴보다가 더 살펴봐도 결과가 좋아질 수 없는 순간에 탐색을 멈춰 외부 호출 수를 줄이면서도 추천 결과의 정확성은 그대로 유지하고, 경로상 가장 싼 주유소만 단순히 고르는 방식보다 실제 비용을 줄인다.

## Abstract

This paper proposes a method to recommend a route through a cost-optimal gas station, expanding the search space — previously confined to the immediate vicinity of the planned route — to the minimum bounding rectangle (MBR) enclosing the route, enabling efficient identification of the most cost-effective station. Candidates priced higher than the minimum on-route price are pruned in advance, since they cannot beat that baseline even with zero detour. The actual driving route through each remaining candidate is then recomputed, and candidates are ranked by a score that combines fuel price with the fuel and time cost of the detour. Evaluation proceeds in ascending order of price, and the search terminates the moment further evaluation can no longer improve the result. The system thus bounds external routing-API calls while preserving recommendation correctness, and yields tangible cost savings over naively picking the cheapest gas station along the route.
...
유가는 다양한 요인에 의해 변동하며, 일상적으로도 상승하는 현상이 관찰된다. 최근 미국·이란 간 군사적 충돌과 호르무즈 해협을 둘러싼 긴장으로 국제 원유 공급망의 불확실성이 커지고 있으며, 국제 유가의 변동성은 운전자의 이동 비용에 직접적인 영향을 미친다.

휘발유 수요는 가격에 둔감(비탄력적)하여[1], 소비자는 이동 자체를 줄이기보다 비용을 낮추는 방향으로 선택을 조정하는 경향을 보인다. 또한 주유소 선택 요인에 관한 연구[2]에서 가격은 재방문 의도에 유의한 음의 영향을 미치는 요인으로 보고되며, 주유소 간 리터당 가격 차이는 소비자의 주유소 선택에 중요하게 작용한다.
