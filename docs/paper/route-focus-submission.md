...
기본 경로로부터의 직선 거리만으로 후보를 거르는 방식은 한계가 분명하다. 직선상 멀어 보여도 경유 경로를 새로 계산하면 우회 비용이 작은 후보가 발견되는데, 이런 후보들이 사전에 배제되면 검색에서 누락되는 오류가 발생한다. 본 연구는 탐색 공간을 관대하게 열되, 후보 수의 추이를 관찰하며 가격 하한과 호출 수 상한을 결합한 단계적 점진 압축으로 좋은 후보를 보존하면서 외부 경로 탐색 API 호출 수를 제어한다. 단계 구성은 표 2와 같다.
...
본 시스템은 4개 시나리오 모두에서 기존 서비스 대비 가격 우위를 보였다. 우위 폭은 10~77원/L 범위였으며, 가장 큰 사례 4의 경우 50 L 기준 약 3,850원의 절감에 해당한다. 기존 서비스에서 옵션 조합으로 근접한 결과를 얻을 수 있는 경우라도 본 시스템은 별도 조작 없이 비용 통합 점수로 후보를 제시한다는 점에서 차별성을 가진다.
