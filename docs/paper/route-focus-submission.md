...
실제 경로상 주유소와 경로를 벗어나 접근 가능한 주유소 간에는 리터당 가격 차이가 존재해, 추가 이동 비용을 고려해도 우회 후보가 더 경제적인 경우가 있다. 경로 폴리라인(도로 모양을 표현한 좌표 점들의 줄)이 지나는 구간만 후보로 삼는 기존의 국소 탐색은 이러한 후보를 배제하므로 비용 최소화 관점에서 한계가 있다. 본 연구는 기본 경로를 감싸는 최소경계사각형(MBR)에 여유 영역(buffer)을 더한 공간으로 탐색을 확장해, 각 후보 주유소를 경유하는 경로를 평가해 최적의 주유소를 찾는 방식을 제안한다.
...
표 1에서 기존 서비스는 기본 경로 주변에 한정된 주유소를 가격순으로 나열할 뿐, 경로 외곽 후보 탐색이나 우회·주유 예정량을 함께 반영한 비용 산출은 다루지 않는다.
