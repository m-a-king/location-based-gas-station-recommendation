...
제안 시스템은 출발지·도착지 입력 UI, 추천 알고리즘, 선택된 경유 경로의 외부 내비게이션 앱 연동을 포함하는 운용 가능한 형태로 구현되었으며, 본 절은 그 핵심인 추천 알고리즘을 다룬다. 추천 알고리즘은 네 단계로 동작한다. 기본 경로에 후보 주유소를 경유지로 삽입한 경유 경로를 재조회해 각 후보의 실측 우회 비용을 산출하는 구조를 가리킨다.
