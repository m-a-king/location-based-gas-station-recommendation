...
후보 주유소를 기본 경로로부터의 직선 거리만으로 거르면, 직선상 멀어 보여도 그 주유소를 경유지로 두고 경로를 새로 계산했을 때 우회 비용이 작은 후보를 검색에서 누락하는 오류(false negative)가 발생한다. 반대로 탐색 공간을 너무 넓게 잡으면 외부 API 호출이 폭증한다. 본 연구는 후보 수를 관찰하며 식 (2)의 가격 하한과 외부 API 호출 수 강제 상한을 결합해 단계적으로 점진 압축하는 전략을 채택한다(표 2).
