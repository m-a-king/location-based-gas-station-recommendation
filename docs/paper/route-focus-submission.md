...
본 논문은 주유소 경유 경로 추천에서 기본 경로상으로 탐색 공간을 한정하던 기존 방식 대신, 기본 경로를 감싸는 최소경계사각형과 여유 영역까지 확장해 최적의 주유소를 추천하는 방법을 제안한다. 확장된 공간에서 후보 주유소를 수집한 뒤 기본 경로상 최저가를 기준으로 비효율적인 후보를 사전 제거하고, 각 후보의 경유 경로를 조회해 주유 비용과 우회로 인한 연료·시간 비용을 통합한 점수로 순위를 결정한다. 비용 개선이 불가능한 시점에 탐색을 조기 종료해 외부 API 호출을 줄이면서도 추천 결과의 최적성을 유지하며, 기존 방식 대비 비용 절감을 달성한다. 본 시스템은 사용자 입력부터 외부 내비게이션 앱 연동까지를 포함한 운용 가능한 형태로 구현되었다.
