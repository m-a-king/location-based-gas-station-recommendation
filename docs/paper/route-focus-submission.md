...
경유 재조회는 외부 경로 탐색 API 호출을 동반하므로 가능한 최소로 수행해야 한다. 후보를 가격 오름차순으로 탐색하면서 상위 $k$개가 확보된 이후, 어느 시점에서든 식 (3)이 충족되면 탐색을 즉시 종료한다.

$$
p_i \ell \;>\; \text{score}^{(k)}_{\text{best}} \tag{3}
$$

이후 후보는 식 (2)에 따라 점수가 $p_i \ell$ 이상이므로 상위 $k$개를 갱신할 수 없으며, 따라서 반환 결과의 최적성은 보존된다. 가격 오름차순 탐색은 점수가 가격에 가장 가까운 경로상 후보부터 보는 순서이므로 종료 조건을 일찍 달성한다.
