...
단순히 직선 거리만으로 후보를 거르는 방식은 한계가 분명하다. 직선상 멀어 보여도 경유 경로를 새로 계산하면 우회 비용이 작은 후보가 적지 않게 발견되는데, 이런 후보들이 사전에 배제되면 검색에서 누락되는 오류(false negative)가 발생한다. 본 연구는 탐색 공간을 관대하게 열되, 후보 수의 추이를 관찰하며 식 (2)의 가격 하한과 호출 수 상한을 결합한 단계적 점진 압축으로 좋은 후보를 보존하면서 외부 API 호출을 제어한다(표 2).
