...
일반적으로 경로는 도로의 기하학적 형상을 반영하여, 일련의 직선 세그먼트들이 연결된 폴리라인(Polyline) 형태로 모델링된다. 기본 경로상 주유소와 경로를 벗어나 접근할 수 있는 주유소 간에는 리터당 가격 차이가 존재해, 추가 이동 비용을 고려해도 우회 후보가 더 경제적인 경우가 있다.
