...
국소 탐색은 폴리라인으로부터의 직선 거리를 기준으로 후보를 필터링하지만, 도로망에서는 직선 거리와 실제 진출입 우회 거리가 크게 어긋날 수 있다. 예를 들어 폴리라인에서 직선 3 km 떨어진 주유소가 진출입 구조상 500 m 우회로 접근되는 사례가 자주 관찰된다(그림 2). 본 연구는 탐색 범위를 경로를 포함하는 MBR로 확장하고, 내부 후보 각각에 대해 경로를 재조회해 실측 우회 비용으로 순위를 정한다.
...
표 3은 **⟨실측 교체 예정: 수집 일자 YYYY-MM-DD, 대상 지역⟩**에서 오피넷과 오일나우를 통해 수집한 실측 가격과 Kakao Directions API 응답을 바탕으로, 세 건의 기본 경로에 대해 경로상 최저가 후보(ON_ROUTE)와 비용 인지 우회 후보(OFF_ROUTE)를 비교한 결과이다(유종: 휘발유, $\ell = 40$ L, $\eta = 10$ km/L). 각 사례의 우회 허용 상한은 $\min(\text{기본경로} \times 0.3,\ 10)$ km로 산정하였다.
