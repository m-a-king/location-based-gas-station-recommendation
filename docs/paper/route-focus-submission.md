...
경유 재조회는 각 후보의 실측 우회 비용을 외부 경로 탐색 API로 산출해 식 (1)의 점수를 매기는 과정이며, API 호출을 동반하므로 가능한 최소로 수행해야 한다. 본 시스템은 후보를 가격 오름차순으로 살피며, 추천 개수 $k$(본 시스템은 $k=3$으로 운용)만큼 상위 후보를 확보한 뒤부터 종료 조건을 검사한다. 새 후보의 가격 하한 $p_i \ell$이 확보된 상위 $k$개 중 가장 나쁜 점수 $\text{score}^{(k)}_{\text{best}}$를 초과하면, 즉 식 (3)이 성립하면 탐색을 즉시 종료한다.
