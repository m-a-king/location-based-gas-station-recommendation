...
2. 후보 수집: 기본 경로의 MBR에 우회 허용 상한의 두 배만큼 여유 영역을 더해 탐색 공간을 구성하고 내부 주유소를 DB에서 조회한다. 후보가 과다하면 단계적으로 압축한다(3.4절).
3. 경유 재조회: 각 후보를 경유지로 하는 경로를 외부 경로 탐색 API로 재조회해 실측 우회 거리·시간을 얻으며, 가격 하한 가지치기로 호출 수를 제한한다(3.5절).
4. 순위화: 비용 통합 점수로 오름차순 정렬해 최대 3개까지 반환한다(유효 후보가 부족하면 0~2개). 3.5절의 가지치기로 재조회를 조기 종료해도 반환 결과의 최적성은 수학적으로 보존된다.
...
기본 경로로부터의 직선 거리만으로 후보를 거르는 방식은 한계가 분명하다. 직선상 멀어 보여도 경유 경로를 새로 계산하면 우회 비용이 작은 후보가 적지 않게 발견되는데, 이런 후보들이 사전에 배제되면 검색에서 누락되는 오류(false negative)가 발생한다. 본 연구는 탐색 공간을 관대하게 열되, 후보 수의 추이를 관찰하며 가격 하한과 호출 수 상한을 결합한 단계적 점진 압축으로 좋은 후보를 보존하면서 외부 경로 탐색 API 호출 수를 제어한다. 단계 구성은 표 2와 같다.
...
경유 재조회는 각 후보의 실측 우회 비용을 외부 경로 탐색 API로 산출해 식 (1)의 점수를 매기는 과정이며, API 호출을 동반하므로 가능한 최소로 수행해야 한다. 본 시스템은 후보를 가격 오름차순으로 살피며, 추천 개수 $k$(본 연구는 3)만큼 상위 후보를 확보한 뒤부터 종료 조건을 검사한다. 새 후보의 가격 하한 $p_i \ell$이 확보된 상위 $k$개 중 가장 나쁜 점수 $\text{score}^{(k)}_{\text{best}}$를 초과하면, 즉 식 (3)이 성립하면 탐색을 즉시 종료한다.
...
본 시스템은 4개 시나리오 모두에서 기존 서비스 대비 가격 우위를 보였다. 사례 1(동대구역-국립금오공과대학교)에서 28원/L(40 L 기준 약 1,120원 절감), 사례 2(잠실역-수원역)에서 오일나우 대비 10원/L·오피넷 대비 24원/L, 사례 3(서초IC-서대전IC)에서 15원/L, 사례 4(대구역-동대구역)에서 77원/L의 우위가 확인되었다. 우위 폭이 가장 큰 사례 4는 도심 외곽의 저가 주유소가 기본 경로상 후보로는 노출되지 않는 상황에서 MBR 확장이 절감 효과를 크게 가져옴을 시사한다. 기존 서비스에서 옵션 조합으로 근접한 결과를 얻을 수 있는 경우라도 본 시스템은 별도 조작 없이 비용 통합 점수로 후보를 직접 제시한다는 점에서 자동화 측면의 차별성을 가진다.
...
본 논문은 기본 경로상으로 후보를 한정하는 기존 방식의 한계를 지적하고, 경유 경로 재탐색을 통한 비용 통합 점수로 주유소 추천을 제안하였다. 본 연구는 입지 최적화의 deviation tolerance[4]를 운용 시점 추천으로 전환하였으며, 실측 사례 4개 시나리오 모두에서 기존 서비스 대비 가격 우위를 확인하였다. 또한 출발지·도착지 입력부터 외부 내비게이션 앱 연동까지를 포함한 운용 가능한 형태로 구현하여 사용자가 추천 결과를 즉시 활용할 수 있도록 하였다. 본 연구는 단일 시점·단일 유종 검증에 기반하므로, 향후 시간대·요일에 따른 우회 비용 변동을 누적 데이터로 반영하고, 정적 우회 허용 상한과 [5]의 평균 수용 우회 범위를 사용자의 채택·거부 신호로 학습하여 개인화 추천으로 확장할 계획이다.
