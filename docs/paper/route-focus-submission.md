...
우선 1단계는 공간을 관대하게 열어 후보를 폭넓게 수집한다. 다음 2단계는 경로상 후보의 최저가를 가격 상한으로 두고, 이를 초과하는 후보를 배제한다. 그 근거는 식 (1)의 모든 항이 비음수이므로 식 (2)가 성립한다는 점이다.
...
단계 3의 경유 경로 호출은 외부 경로 탐색 API에 의존하므로 가능한 최소로 수행해야 한다. 식 (2)의 하한을 활용하면 다음과 같은 종료 조건이 성립한다. 후보를 가격 오름차순으로 탐색하면서 상위 $k$개가 확보된 이후 어느 시점에서든 식 (3)이 성립하면,
