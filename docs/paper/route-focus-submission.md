...
경유 재조회는 각 후보의 실측 우회 비용을 외부 경로 탐색 API로 산출해 식 (1)의 점수를 매기는 과정이며, API 호출을 동반하므로 가능한 최소로 수행해야 한다. 본 시스템은 후보를 가격 오름차순으로 살피며, 추천 개수 $k$(본 연구는 3)만큼 상위 후보를 확보한 뒤부터 종료 조건을 검사한다. 그 시점까지 확보된 상위 $k$개 점수 중 가장 나쁜 값을 $\text{score}^{(k)}_{\text{best}}$라 하면, 새로 평가할 후보의 가격 하한 $p_i \ell$이 이 기준치를 이미 넘어선 시점, 즉 식 (3)이 충족될 때 탐색을 즉시 종료한다.

$$
p_i \ell \;>\; \text{score}^{(k)}_{\text{best}} \tag{3}
$$

식 (2)에 따라 후보의 점수는 $p_i \ell$ 이상이므로, 식 (3)이 충족된 후보는 우회 비용을 0으로 가정해도 이미 확보된 $k$번째 점수를 넘는다. 게다가 가격 오름차순 탐색에서는 그 이후 후보 모두 가격이 더 비싸 식 (3)이 자동으로 충족된다. 따라서 탐색을 즉시 종료해도 어떤 후보도 상위 $k$개를 갱신할 수 없어 반환 결과의 최적성이 보존된다.
