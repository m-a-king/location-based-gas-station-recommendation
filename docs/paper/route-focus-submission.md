...
기존 주유소 탐색은 경로가 지나는 구간만 후보로 고려하므로 비용 최적화 관점에서 한계가 있다. 본 연구는 기본 경로를 탐색 공간 결정 기준으로 활용하여, 그 경로를 감싸는 최소경계사각형(MBR)과 여유 영역(buffer) 안에서 찾은 각 후보 주유소를 경유지로 삼아 경유 경로를 새로 계산한 뒤 비용 기준으로 평가하는 방법을 제안한다.
...
본 시스템은 4개 시나리오 모두에서 기존 서비스 대비 가격 우위를 보였다. 우위 폭은 10~77원/L 범위였으며, 가장 큰 사례 4의 경우 50 L 기준 약 3,850원의 절감에 해당한다. 기존 서비스에서 옵션 조합으로 근접한 결과를 얻을 수 있는 경우라도 본 시스템은 별도 조작 없이 비용 통합 점수로 후보를 직접 제시한다는 점에서 자동화 측면의 차별성을 가진다.
