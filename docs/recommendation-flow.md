...
- **최저시급**: 2026년 기준 10,320원/시간 (반경 기반에서는 우회 시간 0으로 항 생략)
...
flowchart TD
    ...
    C --> D
    D["③ 좌표 역변환<br/>KATEC → WGS84<br/>응답 및 내부 처리 기준이 WGS84이므로"]
    D --> E
    E["④ 점수 계산<br/>각 주유소에 점수 공식 적용<br/>거리 = OPINET이 반환한 직선거리"]
    E --> F
    F["⑤ 정렬 및 선택<br/>점수 오름차순 → 상위 limit개"]
    F --> G([응답 반환])
```
...
flowchart TD
    ...
    D -->|경로에서 2km 초과<br/>현실적 우회 범위 초과| DISC1[제외]
    D -->|경로에서 2km 이내| E
    ...
    H{"⑦ price lower bound 검사<br/>limit개 미만이면 검사 생략<br/>price x 주유량 > 현재 limit번째 점수?"}
    H -->|예: 가격이 높을수록 점수도 높으므로<br/>이후 후보 전부 탈락| J
    H -->|아니오| I

    I["⑧ Kakao 경유 경로 조회<br/>출발 → 주유소 → 도착<br/>우회 거리 = 경유 거리 - 기본 거리<br/>우회 시간 = 경유 시간 - 기본 시간<br/>음수면 0으로 보정: Kakao 측정 오차 대응<br/>점수 = 주유비 + 우회 연료비 + 우회 시간 x 최저시급"]
    I --> H2["결과 추가 후 limit번째 점수 갱신<br/>이후 pruning 기준으로 사용"]
    H2 --> H
