...
class OpinetCsvDownloader(
    ...
    fun downloadCurrentPriceCsv(): ByteArray {
        val sessionId = login()
        visitDownloadPage(sessionId)   // 세션이 다운로드 페이지를 방문해야 서버가 허용
    ...
    private fun doLogin(userId: String, encryptedPwd: String, sessionId: String, wmonId: String): String {
        ...
        return when {
            ...
            responseCode in 200..302 -> {
                // 일부 응답은 쿠키 없이 200/302만 반환하고 기존 세션을 그대로 사용
    ...
    private fun fetchNetFunnelKey(): String {
        ...
        // nfl.opinet.co.kr 는 TLS_RSA_* 암호스위트만 지원하며, Java 17+ 에서는 기본 비활성화,
        // Java 25+ 에서는 완전 제거됨. curl 로 우회한다.
    ...
    private fun postDownload(netFunnelKey: String, sessionId: String): ByteArray {
        ...
        // PAGE_DIV_5: 유가정보(현재 판매가격) CSV 다운로드 (전국, MS949 인코딩)
        // SIDO_NM/SIGUN_NM 은 빈 문자열이어야 전국 조회 가능
        val body = listOf(
            "LPG_CD"        to "A",
            "DATE_DIV_CD"   to "",
