 ...
 */
enum class CandidateSelectionStage(val description: String) {
    /** polyline MBR + 동적 buffer 내 전체 주유소 수집 */
    POLYLINE_MBR("polyline MBR + 동적 buffer (기본 경로의 30%, 최대 10km)"),

    /** polyline 직선 거리 ≤ 2km 필터로 dead-zone 후보 제거 */
    TIGHT_CORRIDOR("polyline distance ≤ 2km 필터"),

    /** 가격 오름차순 상위 30개 하드캡 — Kakao 호출 수 절대 상한 보장 */
    PRICE_CAPPED("가격 오름차순 상위 30개 하드캡")
}
