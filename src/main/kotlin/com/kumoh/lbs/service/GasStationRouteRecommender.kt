...
class GasStationRouteRecommender(
    ...
    fun recommend(
        ...
        val polyline = route.extractPolyline()
        if (polyline.isEmpty()) {
            throw IllegalStateException("경로 데이터가 비어 있습니다.")
        }

        val baseDistance = route.summary?.distance
            ?: throw IllegalStateException("경로 거리 정보가 없습니다.")
        ...
        val uniqueStations = samplePoints
            .flatMap { point ->
                val coord = Coordinate.fromWgs84(point)
                opinetClient.searchByRadius(coord, SEARCH_RADIUS, fuelType, SortType.PRICE)
            }
            .distinctBy { it.id }
        ...
        val refinedCandidates = preliminaryCandidates.mapNotNull { scored ->
            ...
            val viaDistance = viaRoute?.summary?.distance
            if (viaDistance == null) {
                logger.debug { "경유 경로 조회 실패: ${scored.station.name}, 1차 점수 유지" }
                return@mapNotNull scored
            }

            val actualDetour = (viaDistance - baseDistance).coerceAtLeast(0).toDouble()
    ...
    private fun samplePolyline(polyline: List<Coordinate.Wgs84>, intervalMeters: Double): List<Coordinate.Wgs84> {
        ...
        for (i in 1 until polyline.size) {
            val dist = haversineMeters(polyline[i - 1], polyline[i])
            accumulated += dist
    ...
    private fun calculateDetourDistance(
        ...
        polyline: List<Coordinate.Wgs84>
    ): Double {
        val stationPoint = stationLocation.wgs84
        var minDistance = Double.MAX_VALUE

        for (i in 0 until polyline.size - 1) {
            val dist = pointToSegmentDistance(stationPoint, polyline[i], polyline[i + 1])
            if (dist < minDistance) {
                minDistance = dist
            }
        }
    ...
    private fun haversineMeters(a: Coordinate.Wgs84, b: Coordinate.Wgs84): Double {
